...
- [ ] enforcer agent — A trusted agent owned by the repo owner that runs on the Anthropic API (not Claude Code CLI — needs to operate without a human session). When a PR's branch changes after votes already passed, governance pauses and notifies the enforcer. Enforcer diffs the two SHAs (`approved-sha` vs `new-sha`), verifies the delta is purely conflict resolution with no new logic, and posts `enforcer:approve` or `enforcer:block`. Governance resumes on approve; PR closes on block. Original votes stand either way. Requires: Anthropic API key, always-on hosting (Railway or similar), a webhook or poll mechanism to receive notifications from the game server.
